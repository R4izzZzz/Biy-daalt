...
const Navbar = () => {
  ...
  return (
    <header className="h-12 bg-white dark:bg-gray-950 shadow-md fixed top-0 left-0 right-0 z-50 transition-colors duration-300">
      <nav className="container mx-auto px-4 py-3 flex justify-between items-center">
        {/* Logo Section */}
        <div className="flex items-center space-x-2">
          <a
            href="/"
            className="flex items-center text-2xl font-bold pl-10 dark:text-green-500 hover:text-white dark:hover:text-white transition"
          >
            Welcome!!!
          </a>
        </div>
        ...
        <div className="hidden md:flex items-center space-x-6">
          {NavLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="flex items-center pr-8 font-semibold text-gray-700 dark:text-green-500 hover:text-white dark:hover:text-white transition capitalize"
            >
              {link.label}
            </a>
          ))}
